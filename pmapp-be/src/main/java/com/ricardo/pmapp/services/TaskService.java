...
public class TaskService implements TaskServiceI {
    ...
    @Override
    @Transactional
    public Task create(Task task, UserPrincipal requester) throws TaskCreationException, AccessDeniedException {
        ...
        if (task.getAssignee() != null && task.getAssignee().getUsername() != null &&
            ...
            Optional<Project> existingProject = projectRepository.findById(task.getProject().getCode());
            ...
            } else if (!canModifyProject(existingProject.get(), requester)) {
    ...
    @Override
    @Transactional
    public Task update(Task task, UserPrincipal requester) throws TaskNotFoundException, TaskUpdateException, AccessDeniedException {
        // Check that provided Task exists first
        Task existingTask = getByCode(task.getCode());
        ...
        if (task.getAssignee() != null && task.getAssignee().getUsername() != null &&
            ...
            Optional<Project> optExistingProject = projectRepository.findById(task.getProject().getCode());
            if (!optExistingProject.isPresent()) {
                // If a Project is passed to the Task, it should exist
                throw new TaskUpdateException(String.format(ExceptionMessages.PROJECT_NOT_EXISTING,
                        task.getProject().getCode()));
            } else if (!canModifyTask(existingTask, optExistingProject.get(), requester)) {
                // Check that user has rights to modify provided Task
                throw new AccessDeniedException(String.format(ExceptionMessages.FORBIDDEN_PROJECT,
                        optExistingProject.get().getName()));
            }
            existingProject = optExistingProject.get();
        }

        if (requester.getRole() == Role.Developer
                || (requester.getRole() == Role.ProjectManager && !canModifyProject(existingProject, requester))) {
            // Ignore forbidden fields if requester is a Developer or a Project Manager not owning the proj
            task.setProject(existingTask.getProject());
            task.setDeadline(existingTask.getDeadline());
            if (task.getAssignee() != null && !task.getAssignee().getUsername().equals(requester.getUsername())) {
    ...
    public void deleteByCode(Long code, UserPrincipal requester)
        ...
        // Check that user has rights to modify provided Task
        if (!canModifyProject(existingTask.getProject(), requester)) {
            throw new AccessDeniedException(String.format(ExceptionMessages.FORBIDDEN_PROJECT,
    ...
    public void deleteByProject(Long code, UserPrincipal requester)
        ...
        // Check that user has rights to modify Project (and thus Tasks of this project)
